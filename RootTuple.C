...
void RootTuple::Loop() {
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      int StdHepN = (int)(Pz->size());
		...
		for (int i = 0; i < StdHepN; i ++) {
         ...
         // Final state hadron
				...
				TLorentzVector GiBUUParticle(Px->at(i),Py->at(i),Pz->at(i),E->at(i));
				double GiBUUMomentum = GiBUUParticle.Rho();

				CounterFinalStateParticles++;

   	   //----------------------------------------//

         int pdg = barcode->at(i);

			if (pdg == ProtonPdg && GiBUUMomentum > ArrayNBinsProtonMomentum[0]) { 
			...
			if ( TMath::Abs(pdg) == AbsChargedPionPdg && GiBUUMomentum > ChargedPionMomentumThres) { CounterPions++; }

			if ( pdg == NeutralPionPdg) { CounterNeutralPions++; }

			if ( pdg != NeutralPionPdg && fabs(pdg) != AbsChargedPionPdg && tools.is_meson_or_antimeson(pdg) ) 
      ...
      // Final state lepton
		...
		TLorentzVector MuonVector(lepOut_Px,lepOut_Py,lepOut_Pz,lepOut_E);
		double MuonMomentum = MuonVector.Rho(); // GeV / c
		double MuonCosTheta = MuonVector.CosTheta(); 
		double MuonPhi = MuonVector.Phi() * 180. / TMath::Pi(); // deg
		double MuonEnergy = MuonVector.E(); // GeV           
      ...
      //----------------------------------------//
		...
		if (CounterProtons != 1) { continue; }
		if (CounterPions != 0) { continue; }
		if (CounterNeutralPions != 0) { continue; }
      ...
      // Final state proton
		...
		int ProtonIndex = ProtonIndices.at(0);
		TLorentzVector ProtonVector(Px->at(ProtonIndex),Py->at(ProtonIndex),Pz->at(ProtonIndex),E->at(ProtonIndex)); // GeV
		double ProtonMomentum = ProtonVector.Rho(); // GeV / c								
		double ProtonCosTheta = ProtonVector.CosTheta();
		double ProtonPhi = ProtonVector.Phi() * 180. / TMath::Pi(); // deg									 
		double ProtonEnergy = ProtonVector.E(); // GeV
		double ProtonKineticEnergy = ProtonEnergy - ProtonMass_GeV; // GeV 

      //----------------------------------------//

		// Relative Proton - Muon Angles

		double DeltaThetaProtonMuon = (ProtonVector.Vect()).Angle(MuonVector.Vect()) * 180. / TMath::Pi();
		if (DeltaThetaProtonMuon < 0.) { DeltaThetaProtonMuon += 180.; }
		if (DeltaThetaProtonMuon > 180.) { DeltaThetaProtonMuon -= 180.; }

		double DeltaPhiProtonMuon = (MuonVector.Vect()).DeltaPhi(ProtonVector.Vect())* 180. / TMath::Pi(); // Afro
		if (DeltaPhiProtonMuon < 0.) { DeltaPhiProtonMuon += 360.; }
		if (DeltaPhiProtonMuon > 360.) { DeltaPhiProtonMuon -= 360.; }

      //----------------------------------------//

		STV_Tools stv_tool(MuonVector.Vect(),ProtonVector.Vect(),MuonEnergy,ProtonEnergy);
      ...
      //----------------------------------------//
		...
		if (
			...
			if (
				...
				if (evType == 1) { CounterSTVlikeQEEventsPassedSelection++; }
				if (evType == 2) { CounterSTVlikeMECEventsPassedSelection++; }
				if (evType == 11 || evType == 12 || evType == 13) { CounterSTVlikeRESEventsPassedSelection++; }
				if (evType == 26) { CounterSTVlikeDISEventsPassedSelection++; }
